...
export default class Supervisor {
  ...
  startTunnels() {
    ...
    return new Promise((resolve, reject) => {
      ...
      Promise
        .map(this.tunnels, (tunnel) => tunnel.start(tunnel))
        .then(() => {
  ...
  stopTunnels() {
    return new Promise((resolve, reject) => {
      ...
      Promise
        .map(this.tunnels, (tunnel) => tunnel.stop(tunnel, true))
        .then(() => {
  ...
  supervise() {
    ...
    return new Promise((resolve, reject) => {
      ...
      Promise
        .map(this.tunnels, (tunnel) => tunnel.monitor(tunnel))
        .then(() => resolve())
        .catch((err) => {
  ...
  scheduleRestart() { // restart all tunnels
    schedule.scheduleJob(this.restartCron, () => {
      ...
      Promise
        ...
        .catch((err) => {
          logger.err(util.format("Some tunnels are wrong\n%s", err));
        });
    })
  }
};