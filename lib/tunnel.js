...
const privateOptions = Array.of("id", "pidTempPath", "restartCron", "statsdHost", "statsdPort", "statsdPrefix", "statsQueue", "statsSwitch", "statsD");
        ...
        self.sauceConnectLauncher(self.options, (err, sauceConnectProcess) => {
          if (err) {
            ...
            if (retries < RETRY_LIMIT) {
              logger.warn(util.format("<retried %s/%s starting...> Sauce Tunnel #%s still isn't started", retries + 1, RETRY_LIMIT, self.index));
